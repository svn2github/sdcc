...
/* --lang=xxx */
static int demangle_lang = DMGL_ANSI | DMGL_PARAMS;
...
Usage: %s [-ahifCdDprRtTxsSlw] [-b bfdname] [-m machine] \n\
  ...
  fprintf (stream, _("\
       ...
       [--prefix-addresses] [--[no-]show-raw-insn] [--demangle] [--lang=language]\n\
...
#define OPTION_LANG (OPTION_ADJUST_VMA + 1)
     ...
     asymbol *sym;
{
  ...
  if (! do_demangle || name[0] == '\0')
	...
	++name;
      ...
      alloc = cplus_demangle (name, demangle_lang);
      if (alloc == NULL)
	print = name;
      else
	print = alloc;
	  ...
	  if (cur_bfd != NULL)
	      ...
	      if (do_demangle && name != NULL && *name != '\0')
		{
		  ...
		  if (bfd_get_symbol_leading_char (cur_bfd) == *n)
		    ++n;
		  alloc = cplus_demangle (n, demangle_lang);
		  if (alloc != NULL)
		    (*current)->name = alloc;
		  else
		    (*current)->name = n;
