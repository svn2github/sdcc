...
/* List invented to support redefine_sym */

struct redefine_list_node
{
  char *source;
  char *target;
  struct redefine_list_node *next;
};
...
static char* lookup_sym_redefinition PARAMS((char *source));
...
static struct redefine_list_node *redefine_sym_list = NULL;
...
Usage: %s [-vVSpgxX] [-I bfdname] [-O bfdname] [-F bfdname] [-b byte]\n\
  ...
  fprintf (stream, _("\
       ...
       [--redefine-sym=old_form=new_form] [-W symbol]\n\
...
/* Choose which symbol entries to copy; put the result in OSYMS.
   ...
   Return the number of symbols to print.  The redefine_sym option 
   is just another filter, not unlike remove_leading_char */
...
{
	char* result;
	struct redefine_list_node *list;
	
	if(!redefine_sym_list) return source;

	result = source;
	list = redefine_sym_list;
	...
	const char *target;
{
	...
	struct redefine_list_node *list;
	struct redefine_list_node *new_node;
    		
	new_node = (struct redefine_list_node*) xmalloc (sizeof(struct redefine_list_node));
	...
	else
	{
		...
		while(list != NULL)
		{
			...
			if(strcmp(source, list->source) == 0)
			{
				fprintf(stderr, "redefine-sym:  Multiple redefinition of symbol \"%s\"\n", source);
				exit(1);
			}

			if(strcmp(target, list->target) == 0)
			{
				fprintf(stderr, "redefine-sym: Symbol \"%s\" is target of more than one redefinition", target);
				exit(1);
			} 
