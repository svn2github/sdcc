...
#define OPTION_LANG (OPTION_TARGET + 1)
...
static struct option long_options[] =
{
  ...
  {"lang", required_argument, 0, OPTION_LANG},
  {0, no_argument, 0, 0}
};

...
Usage: %s [-aABCDglnopPrsuvV] [-t radix] [--radix=radix] [--target=bfdname]\n\
       ...
       [--format={bsd,sysv,posix}] [--demangle] [--no-demangle] [--lang=language]\n\
     ...
     bfd *abfd;
{
  if (do_demangle && *name)
	...
	name++;
      ...
      res = cplus_demangle (name, demangle_lang);
      if (res)
	{
	  printf (format, res);
	  free (res);
