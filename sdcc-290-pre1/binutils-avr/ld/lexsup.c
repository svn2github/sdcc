...
/* --lang=xxx */
int demangle_lang = DMGL_ANSI | DMGL_PARAMS;
...
#define OPTION_LANG			(OPTION_IGNORE + 1)
#define OPTION_MAP			(OPTION_LANG + 1)
...
#define OPTION_MULTILIB_DIR		(OPTION_FINI + 1)
...
static const struct ld_option ld_options[] =
{
  ...
  { {"lang", required_argument, NULL, OPTION_LANG},
      '\0', "LANGUAGE", N_("Set language for demangle"), TWO_DASHES },
     ...
     char **argv;
{
  ...
  while (1)
	...
	case 'l':
	  ...
	  break;
	case OPTION_LANG:
	  if (strcasecmp (optarg, "gnat") == 0)
	    demangle_lang = DMGL_GNAT;
	  break;
	case 'M':
	  config.map_filename = "-";
	  break;
	...
	case OPTION_MULTILIB_DIR:
	  command_line.multilib_dir = buystring (optarg);
	  break;
	case OPTION_TBSS:
