...
add_to_lit_pool (expressionS *exx, char *name, int sz)
{
  ...
  if (8 == sz && NULL == longlong_poolP) longlong_poolP = symbol_make_empty();
  else if (4 == sz && NULL == word_poolP) word_poolP = symbol_make_empty();
  else if (2 == sz && NULL == short_poolP) short_poolP = symbol_make_empty();
  else if (1 == sz && NULL == byte_poolP) byte_poolP = symbol_make_empty();
  ...
  while (lit_count < next_literal_pool_place)
    {
      ...
      if (    (NULL != literals[lit_count].sym_name)
        ...
        { break; }
      if (sz == literals[lit_count].size) offset_in_pool += sz;
      lit_count ++;
    }
   ...
   */
  if (8 == sz) exx->X_add_symbol = longlong_poolP;
  else if (4 == sz) exx->X_add_symbol = word_poolP;
  else if (2 == sz) exx->X_add_symbol = short_poolP;
     ...
     int ignore;
{
  ...
  for (litsize=8; litsize; litsize /=2) {
    ...
    if (NULL == current_poolP) continue;
