...
sources with different FOSS licenses. See the sdccman.pdf document,
chapter "SDCC Suite Licenses" for details.

The code or object files generated by SDCC suite are not licensed, so
they can be used with FLOSS or proprietary (closed source) applications.

The great majority of sdcc run time libraries are licensed under
