...
{
  wassert (size <= sizeof (unsigned long));
  ...
  for (i = 0, started = FALSE; i < size;)
    {
      ...
      // We can use incw / decw only for the only, top non-zero word, since it neither takes into account an existing carry nor does it update the carry.
        ...
        (aopInReg (result_aop, i, X_IDX) || aopInReg (result_aop, i, Y_IDX)) &&
        (right_aop->type == AOP_LIT || right_aop->type == AOP_IMMD || aopOnStackNotExt (right_aop, i, 2) /* Bug #2227 || right_aop->type == AOP_DIR && i + 1 < right_aop->size */))
        {
        ...
        (aopOnStackNotExt (right_aop, i, 2) || right_aop->type == AOP_LIT || right_aop->type == AOP_IMMD /* Bug #2227 || right_aop->type == AOP_DIR && i + 1 < right_aop->size*/) &&
